...
Reset on 2026-07-22. This file previously carried the full excavator-project
...
would mislead future sessions into treating stale excavator specifics as
...
excavator and bucket-style gripper). It is being re-implemented into a
**manipulator-based tabletop stacking system**:

- Robot: manipulator arm (UR5e is the current candidate, not yet confirmed), not an excavator.
...
from `/home/inrol-excavator/worksapce/stacking/stacking-tabletop`.

## 2026-07-22 Kickoff

- Repository copied from `stacking-planner` as the starting point for this re-implementation.
- `docs/llm/` wiki and this file were reset to remove excavator/NUC-specific content that no longer applies; hardware-agnostic planning-core vocabulary and config semantics were kept.
...
- Scope which excavator-specific modules (`ros2/`, `planning/execution*.py`, `gui/execution_window.py`, `scripts/nuc/`, `scripts/desktop/execute_*.py`) get deleted vs. adapted vs. rewritten.
