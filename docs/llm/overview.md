...
`../stacking-planner` (an excavator/gripper stacking planner), and is being
...
excavator system and no longer applies; it has been cleared from this wiki
rather than carried forward as stale guidance.

`../stacking-planner/docs/llm/` and `../stacking-planner/docs/context.md`
still describe the excavator system in full detail and remain a useful
...
Stacking-Tabletop re-implements the excavator-based stone-stacking planner as
a tabletop system driven by a robot manipulator (likely a UR5e). Key intended
changes from `stacking-planner`:

- **Robot**: excavator + bucket/gripper -> manipulator arm (UR5e candidate) with an end effector TBD.
...
- `planning/planning.py` diffsim planner setup for grasp/IK/motion (end-effector-specific bindings will need revisiting for a parallel-jaw/manipulator gripper instead of the excavator gripper).
...
- `ros2/` node wrappers: currently modeled around excavator joints/phases and NUC-relayed topics; needs a manipulator-appropriate ROS2 (or other control) interface.
- `planning/execution*.py`, `gui/execution_window.py`, `scripts/desktop/execute_*.py`: excavator-specific execution workflow (pick/place staging, in-hand scan, NUC-mediated scene scan) needs a manipulator-appropriate rewrite.
...
- `planning/`: diffsim planner setup, grasp/IK/regrasp helpers, execution (excavator-specific, to be rewritten), trajectory visualization.
- `perception/`: point-cloud/mesh/[DSF](glossary.md#dsf) reconstruction utilities; scan/scene-id runtime is excavator/NUC-specific and to be redesigned.
- `ros2/`: ROS2 publisher/subscriber node wrappers and control helpers (excavator-specific, to be rewritten for the manipulator).
- `gui/`: Open3D/PySide execution viewer (excavator-specific execution UI, to be adapted).
- `loader/`: h5 and height-map dataset loaders.
- `model/`: stone/excavator/gripper loading and URDF mesh transform helpers (excavator/gripper models to be swapped for manipulator/end-effector models).
- `utils/`: geometry, neural-network, DSF, scheduling, data helpers.
- `scripts/desktop/`: excavator planning/execution entrypoints; will need manipulator equivalents.
- `scripts/nuc/`: excavator NUC-side scanning/pose-id scripts; **not applicable** to this project's desktop-only topology.
- `scripts/simulation/`, `scripts/train/`, `scripts/test/`, `scripts/debug/`: mostly hardware-agnostic simulation/training/debug tooling.
- `assets/`: stone, gripper, excavator meshes/URDFs/PCDs/[DSF](glossary.md#dsf) assets; excavator/gripper assets will need manipulator/end-effector equivalents.
...
- Scope which excavator-specific modules to delete vs. adapt vs. rewrite from scratch.
